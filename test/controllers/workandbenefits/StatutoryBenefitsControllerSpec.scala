...
import forms.workandbenefits.StatutoryBenefitsFormProvider
import models.{Done, NormalMode, UserAnswers}
import navigation.{FakeNavigator, Navigator}
import org.mockito.ArgumentMatchers.any
import org.mockito.Mockito.when
import org.scalatestplus.mockito.MockitoSugar
import pages.workandbenefits.StatutoryBenefitsPage
import play.api.inject.bind
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers._
import services.UserDataService
import views.html.workandbenefits.{StatutoryBenefitsAgentView, StatutoryBenefitsView}

import scala.concurrent.Future

class StatutoryBenefitsControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new StatutoryBenefitsFormProvider()
  val form = formProvider(isAgent = false)
  val agentForm = formProvider(isAgent = true)

  lazy val statutoryBenefitsRoute = controllers.workandbenefits.routes.StatutoryBenefitsController.onPageLoad(NormalMode, taxYear).url

  "StatutoryBenefits Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, statutoryBenefitsRoute).withSession(validTaxYears)

        val result = route(application, request).value

        val view = application.injector.instanceOf[StatutoryBenefitsView]
    ...
    "must return OK and the correct view for a GET for an agent" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[StatutoryBenefitsAgentView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(mtdItId, taxYear).set(StatutoryBenefitsPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, statutoryBenefitsRoute)
    ...
    "must redirect to Journey Recovery for a GET if no existing data is found" in {
      ...
      running(application) {
        val request = FakeRequest(GET, statutoryBenefitsRoute).withSession(validTaxYears).withSession(validTaxYears)
