...
import forms.workandbenefits.ShareSchemesFormProvider
...
import views.html.workandbenefits.{ShareSchemesAgentView, ShareSchemesView}

import scala.concurrent.Future

class ShareSchemesControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new ShareSchemesFormProvider()
  val form = formProvider(isAgent = false)
  val agentForm = formProvider(isAgent = true)

  lazy val shareSchemesRoute = controllers.workandbenefits.routes.ShareSchemesController.onPageLoad(NormalMode, taxYear).url

  "ShareSchemes Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, shareSchemesRoute).withSession(validTaxYears)

        val result = route(application, request).value

        val view = application.injector.instanceOf[ShareSchemesView]
    ...
    "must return OK and the correct view for a GET for an agent" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[ShareSchemesAgentView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(mtdItId, taxYear).set(ShareSchemesPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, shareSchemesRoute)
