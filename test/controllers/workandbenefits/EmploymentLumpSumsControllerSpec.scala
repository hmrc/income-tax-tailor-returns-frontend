...
class EmploymentLumpSumsControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new EmploymentLumpSumsFormProvider()
  val form = formProvider(isAgent = false)
  val agentForm = formProvider(isAgent = true)

  lazy val employmentLumpSumsRoute = controllers.workandbenefits.routes.EmploymentLumpSumsController.onPageLoad(NormalMode, taxYear).url

  "EmploymentLumpSums Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, employmentLumpSumsRoute).withSession(validTaxYears)

        val result = route(application, request).value

        val view = application.injector.instanceOf[EmploymentLumpSumsView]
    ...
    "must return OK and the correct view for a GET for an agent" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[EmploymentLumpSumsAgentView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(mtdItId, taxYear).set(EmploymentLumpSumsPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, employmentLumpSumsRoute)
