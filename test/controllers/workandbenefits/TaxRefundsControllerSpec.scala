...
import forms.workandbenefits.TaxRefundsFormProvider
import models.{Done, NormalMode, UserAnswers}
import navigation.{FakeNavigator, Navigator}
import org.mockito.ArgumentMatchers.any
import org.mockito.Mockito.when
import org.scalatestplus.mockito.MockitoSugar
import pages.workandbenefits.TaxRefundsPage
import play.api.inject.bind
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers._
import services.UserDataService
import views.html.workandbenefits.{TaxRefundsAgentView, TaxRefundsView}

import scala.concurrent.Future

class TaxRefundsControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new TaxRefundsFormProvider()
  val form = formProvider(isAgent = false)
  val agentForm = formProvider(isAgent = true)

  lazy val taxRefundsRoute = controllers.workandbenefits.routes.TaxRefundsController.onPageLoad(NormalMode, taxYear).url

  "TaxRefunds Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, taxRefundsRoute).withSession(validTaxYears)

        val result = route(application, request).value

        val view = application.injector.instanceOf[TaxRefundsView]
    ...
    "must return OK and the correct view for a GET for an agent" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[TaxRefundsAgentView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(mtdItId, taxYear).set(TaxRefundsPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, taxRefundsRoute)
