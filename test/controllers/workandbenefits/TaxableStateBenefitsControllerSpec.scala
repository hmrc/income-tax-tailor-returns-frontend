...
class TaxableStateBenefitsControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new TaxableStateBenefitsFormProvider()
  val form = formProvider(isAgent = false)
  val agentForm = formProvider(isAgent = true)

  lazy val taxableStateBenefitsRoute = controllers.workandbenefits.routes.TaxableStateBenefitsController.onPageLoad(NormalMode, taxYear).url

  "TaxableStateBenefits Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, taxableStateBenefitsRoute).withSession(validTaxYears)

        val result = route(application, request).value

        val view = application.injector.instanceOf[TaxableStateBenefitsView]
    ...
    "must return OK and the correct view for a GET for an agent" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[TaxableStateBenefitsAgentView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(mtdItId, taxYear).set(TaxableStateBenefitsPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, taxableStateBenefitsRoute)
