...
import views.html.StartAgentView
...
class StartControllerSpec extends SpecBase {
  ...
  "Start Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.StartController.onPageLoad(taxYear).url).withSession(validTaxYears)
    ...
    "must return OK and the correct view for a GET for an agent" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[StartAgentView]
