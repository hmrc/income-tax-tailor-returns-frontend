...
class MarriageAllowanceControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val marriageAllowanceRoute = controllers.aboutyou.routes.MarriageAllowanceController.onPageLoad(NormalMode, taxYear).url

  "MarriageAllowance Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, marriageAllowanceRoute).withSession(validTaxYears)
    ...
    "must return OK and the correct view for a GET for an agent" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[MarriageAllowanceAgentView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(mtdItId, taxYear).set(MarriageAllowancePage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, marriageAllowanceRoute)
