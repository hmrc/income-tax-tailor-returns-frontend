...
class FosterCarerControllerSpec extends SpecBase with MockitoSugar {
  ...
  "FosterCarer Controller" - {
    ...
    "must redirect to the next page and update userAnswers when valid data is submitted" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(emptyUserAnswers.set(FosterCarerPage, false).success.value))
          .overrides(
    ...
    "must redirect to the next page and not update userAnswers when the value submitted is the same as the value stored" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(emptyUserAnswers.set(FosterCarerPage, true).success.value))
          .overrides(
            bind[Navigator].toInstance(new FakeNavigator(onwardRoute)),
            bind[UserDataService].toInstance(mockUserDataService)
          )
