...
import models.propertypensionsinvestments.NonUkInterestDividendsInsurance
...
class NonUkInterestDividendsInsuranceControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  lazy val nonUkInterestDividendsInsuranceRoute = routes.NonUkInterestDividendsInsuranceController.onPageLoad(NormalMode, taxYear).url

  val formProvider = new NonUkInterestDividendsInsuranceFormProvider()
  val form = formProvider(isAgent = false)
  val agentForm = formProvider(isAgent = true)

  "NonUkInterestDividendsInsurance Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, nonUkInterestDividendsInsuranceRoute).withSession(validTaxYears)

        val result = route(application, request).value

        val view = application.injector.instanceOf[NonUkInterestDividendsInsuranceView]
    ...
    "must return OK and the correct view for a GET as an agent" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[NonUkInterestDividendsInsuranceAgentView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(mtdItId, taxYear).set(NonUkInterestDividendsInsurancePage, NonUkInterestDividendsInsurance.values.toSet).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(NonUkInterestDividendsInsurance.values.toSet), NormalMode, taxYear)(request, messages(application)).toString
      }
    }
    ...
    "must populate the view correctly on a GET when the question has previously been answered for an agent" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(agentForm.fill(NonUkInterestDividendsInsurance.values.toSet), NormalMode, taxYear)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, nonUkInterestDividendsInsuranceRoute)
            .withFormUrlEncodedBody(("value[0]", NonUkInterestDividendsInsurance.values.head.toString))
