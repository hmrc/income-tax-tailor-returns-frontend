...
class UkInterestFormProvider @Inject() extends Mappings {

  def apply(isAgent: Boolean): Form[Set[UkInterest]] = {
    val error: String =
      if (isAgent) {
        "ukInterest.agent.error.required"
      } else {
        "ukInterest.error.required"
      }
    Form(
