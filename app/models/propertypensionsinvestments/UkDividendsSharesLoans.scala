...
object UkDividendsSharesLoans extends Enumerable.Implicits {
      ...
      case (value, index) =>
        value match {
          ...
          case CashDividendsFromUkStocksAndShares => CheckboxItemViewModel(
            ...
            index = index,
            value = value.toString
          ).withHint(Hint(content = Text(messages(s"$contentPrefix.dividends.hint"))))
  ...
  def checkboxItems(implicit messages: Messages): Seq[CheckboxItem] =
    getCheckboxItems("ukDividendsSharesLoans")

  def agentCheckboxItems(implicit messages: Messages): Seq[CheckboxItem] =
    getCheckboxItems("ukDividendsSharesLoans.agent")
