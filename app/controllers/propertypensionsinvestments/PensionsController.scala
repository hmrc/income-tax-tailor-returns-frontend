...
class PensionsController @Inject()(
                                        ...
                                        formProvider: PensionsFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: PensionsView,
                                        agentView: PensionsAgentView
  ...
  def onPageLoad(mode: Mode, taxYear: Int): Action[AnyContent] =
    ...
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(PensionsPage) match {
  ...
  def onSubmit(mode: Mode, taxYear: Int): Action[AnyContent] =
    ...
    implicit request =>
      ...
      form(request.isAgent).bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PensionsPage, value))
            _              <- userDataService.set(updatedAnswers, request.userAnswers)
          } yield Redirect(navigator.nextPage(PensionsPage, mode, updatedAnswers))
      )
  }
}
