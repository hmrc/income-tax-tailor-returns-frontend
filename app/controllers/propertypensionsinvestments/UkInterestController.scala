...
class UkInterestController @Inject()(
                                        ...
                                        formProvider: UkInterestFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: UkInterestView,
                                        agentView: UkInterestAgentView
  ...
  def onPageLoad(mode: Mode, taxYear: Int): Action[AnyContent] =
    ...
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(UkInterestPage) match {
  ...
  def onSubmit(mode: Mode, taxYear: Int): Action[AnyContent] =
    ...
    implicit request =>
      ...
      form(request.isAgent).bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(UkInterestPage, value))
            _              <- userDataService.set(updatedAnswers, request.userAnswers)
          } yield Redirect(navigator.nextPage(UkInterestPage, mode, updatedAnswers))
      )
  }
}
