...
class JourneyRecoveryController @Inject()(
  ...
  def onPageLoad(continueUrl: Option[RedirectUrl] = None, taxYear: Int): Action[AnyContent] = identify {
    implicit request =>
      ...
      safeUrl
        .map(url => Ok(continueView(url, taxYear)))
        .getOrElse(Ok(startAgainView(taxYear)))
  }
}
