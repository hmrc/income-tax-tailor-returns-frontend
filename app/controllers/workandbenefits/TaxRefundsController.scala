...
class TaxRefundsController @Inject()(
                                         ...
                                         formProvider: TaxRefundsFormProvider,
                                         val controllerComponents: MessagesControllerComponents,
                                         view: TaxRefundsView,
                                         agentView: TaxRefundsAgentView
  ...
  (identify(taxYear) andThen taxYearAction(taxYear) andThen getData(taxYear) andThen requireData(taxYear)) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(TaxRefundsPage) match {
  ...
  (identify(taxYear) andThen taxYearAction(taxYear) andThen getData(taxYear) andThen requireData(taxYear)).async {
    implicit request =>
      ...
      form(request.isAgent).bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(TaxRefundsPage, value))
            _              <- userDataService.set(updatedAnswers, request.userAnswers)
          } yield Redirect(navigator.nextPage(TaxRefundsPage, mode, updatedAnswers))
      )
  }
}
