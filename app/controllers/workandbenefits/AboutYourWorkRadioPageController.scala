...
class AboutYourWorkRadioPageController @Inject()(
                                         ...
                                         formProvider: AboutYourWorkRadioPageFormProvider,
                                         val controllerComponents: MessagesControllerComponents,
                                         view: AboutYourWorkRadioPageView,
                                         agentView: AboutYourWorkRadioPageAgentView
  ...
  (identify(taxYear) andThen taxYearAction(taxYear) andThen getData(taxYear) andThen requireData(taxYear)) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(AboutYourWorkRadioPagePage) match {
  ...
  (identify(taxYear) andThen taxYearAction(taxYear) andThen getData(taxYear) andThen requireData(taxYear)).async {
    implicit request =>
      ...
      form(request.isAgent).bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AboutYourWorkRadioPagePage, value))
            _              <- userDataService.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(AboutYourWorkRadioPagePage, mode, updatedAnswers))
      )
  }
}
