...
class ShareSchemesController @Inject()(
                                         ...
                                         formProvider: ShareSchemesFormProvider,
                                         val controllerComponents: MessagesControllerComponents,
                                         view: ShareSchemesView,
                                         agentView: ShareSchemesAgentView
  ...
  (identify(taxYear) andThen taxYearAction(taxYear) andThen getData(taxYear) andThen requireData(taxYear)) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ShareSchemesPage) match {
  ...
  (identify(taxYear) andThen taxYearAction(taxYear) andThen getData(taxYear) andThen requireData(taxYear)).async {
    implicit request =>
      ...
      form(request.isAgent).bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(ShareSchemesPage, value))
            _              <- userDataService.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(ShareSchemesPage, mode, updatedAnswers))
      )
  }
}
