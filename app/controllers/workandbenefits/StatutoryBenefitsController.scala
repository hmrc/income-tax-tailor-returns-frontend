...
class StatutoryBenefitsController @Inject()(
                                         ...
                                         formProvider: StatutoryBenefitsFormProvider,
                                         val controllerComponents: MessagesControllerComponents,
                                         view: StatutoryBenefitsView,
                                         agentView: StatutoryBenefitsAgentView
  ...
  (identify(taxYear) andThen taxYearAction(taxYear) andThen getData(taxYear) andThen requireData(taxYear)) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(StatutoryBenefitsPage) match {
  ...
  (identify(taxYear) andThen taxYearAction(taxYear) andThen getData(taxYear) andThen requireData(taxYear)).async {
    implicit request =>
      ...
      form(request.isAgent).bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(StatutoryBenefitsPage, value))
            _              <- userDataService.set(updatedAnswers, request.userAnswers)
          } yield Redirect(navigator.nextPage(StatutoryBenefitsPage, mode, updatedAnswers))
      )
  }
}
