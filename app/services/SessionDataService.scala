...
class SessionDataService @Inject()(sessionDataConnector: SessionDataConnector,
  ...
  def getSessionData()(implicit hc: HeaderCarrier): Future[Either[Unit, Option[SessionData]]] = {
    ...
    if (config.sessionCookieServiceEnabled) {
      ...
      sessionDataCacheResult
        ...
        .map(data => {
          infoLogger(
            "Request to retrieve session data from session cookie service completed successfully"
          )
          data
        }).value
    } else {
      infoLogger("Session cookie service disabled. Returning no data")
      Future.successful(Right(None))
    }
  }

  def getNino()(implicit request: Request[_], hc: HeaderCarrier): Future[Either[Unit, String]] = {
