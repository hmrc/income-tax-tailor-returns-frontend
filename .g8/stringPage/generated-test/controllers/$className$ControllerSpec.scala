...
class $className$ControllerSpec extends SpecBase with MockitoSugar {
  ...
  "$className$ Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(mtdItId, taxYear).set($className$Page, "answer").success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("answer"), NormalMode, taxYear)(request, messages(application)).toString
      }
    }
