...
class $className$ControllerSpec extends SpecBase with MockitoSugar {
  ...
  "$className$ Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(mtdItId, taxYear).set($className$Page, $className$.values.toSet).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill($className$.values.toSet), NormalMode, taxYear)(request, messages(application)).toString
      }
    }
    ...
    "must populate the view correctly on a GET when the question has previously been answered for an agent" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(agentForm.fill($className$.values.toSet), NormalMode, taxYear)(request, messages(application)).toString
      }
    }
